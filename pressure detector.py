...
v3 (15.08.2026): MIN_MINUTE снижен с 15 до 3 — матч сканируется почти сразу
после старта. sportmonks_provider.py уже считает "recent" pressure-окно
корректно на малых минутах (max(0, minute - 15) даёт 0, то есть окно
автоматически берёт все данные с начала матча, пока матч младше 15 минут) —
менять его не требуется, узким местом был именно ранний отсечка здесь.
"""
...
PRESSURE_DETECTOR_VERSION = "v3-min-minute-3-2026-08-15"
...
# Снижено с 15 до 3: раньше первые 15 минут матча детектор вообще
# пропускал (continue) — а именно в этом окне часто забивают ранние голы,
# и данные для сигнала уже есть (см. комментарий в sportmonks_provider.py).
MIN_MINUTE = 3
MAX_MINUTE = 80
